...
class ProductController {
  public async create(req: Request, res: Response): Promise<void> {
    ...
    const Search: string = req.params.Search ? req.params.Search + '%' : '%%';
    const r: any = await query(
      "SELECT a.id,a.name,b.name AS category FROM products AS a LEFT JOIN categories AS b ON a.idcategory = b.id WHERE a.name ILIKE $1 ORDER BY a.name limit 5",
      [Search]
    );
    res.json(r);
  }
